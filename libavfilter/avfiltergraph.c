...
static const AVOption filtergraph_options[] = {
    { "thread_type", "Allowed thread types", OFFSET(thread_type),
     AV_OPT_TYPE_FLAGS,
     {.i64 = AVFILTER_THREAD_SLICE}, 0, INT_MAX, F | V | A,.unit =
     "thread_type" },
    { "slice", NULL, 0, AV_OPT_TYPE_CONST,
     {.i64 = AVFILTER_THREAD_SLICE},.flags = F | V | A,.unit =
     "thread_type" },
                                             ...
                                             const char *name)
{
    ...
    filters =
        av_realloc_array(graph->filters, graph->nb_filters + 1,
                         sizeof(*filters));
    if (!filters)
        return NULL;
        ...
        for (j = 0; j < filt->nb_inputs; j++) {
            if (!filt->inputs[j] || !filt->inputs[j]->src) {
                ...
                av_log(log_ctx, AV_LOG_ERROR,
                       ...
                       pad->name, av_get_media_type_string(pad->type),
                       filt->name, filt->filter->name);
    ...
    for (i = 0; i < graph->nb_filters; i++) {
        ...
        for (j = 0; j < f->nb_outputs; j++) {
            ...
            if (l->type == AVMEDIA_TYPE_VIDEO) {
                ret =
                    av_image_check_size2(l->w, l->h, INT64_MAX, l->format,
                                         0, f);
                if (ret < 0)
                    return ret;
            }
                                           ...
                                           const char *name)
{
    ...
    for (i = 0; i < graph->nb_filters; i++)
        if (graph->filters[i]->name
            && !strcmp(name, graph->filters[i]->name))
                                     ...
                                     AVFilterFormatsConfig * cfg)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        if ((ret = ff_formats_check_pixel_formats(log, cfg->formats)) < 0
            || (ret =
                ff_formats_check_color_spaces(log, cfg->color_spaces)) < 0
            || (ret =
                ff_formats_check_color_ranges(log, cfg->color_ranges)) < 0)
            return ret;
        break;
    ...
    case AVMEDIA_TYPE_AUDIO:
        if ((ret = ff_formats_check_sample_formats(log, cfg->formats)) < 0
            || (ret =
                ff_formats_check_sample_rates(log, cfg->samplerates)) < 0
            || (ret =
...
static int filter_check_formats(AVFilterContext * ctx)
{
    ...
    for (i = 0; i < ctx->nb_inputs; i++) {
        ret =
            filter_link_check_formats(ctx, ctx->inputs[i],
                                      &ctx->inputs[i]->outcfg);
        if (ret < 0)
            return ret;
    }
    for (i = 0; i < ctx->nb_outputs; i++) {
        ret =
            filter_link_check_formats(ctx, ctx->outputs[i],
                                      &ctx->outputs[i]->incfg);
        if (ret < 0)
            return ret;
    }
...
static int filter_query_formats(AVFilterContext * ctx)
{
    ...
    if (filter->formats_state == FF_FILTER_FORMATS_QUERY_FUNC) {
        ...
        if (ctx->nb_inputs > FF_ARRAY_ELEMS(cfg_in_stack)) {
            cfg_in_dyn =
                av_malloc_array(ctx->nb_inputs, sizeof(*cfg_in_dyn));
            if (!cfg_in_dyn)
                return AVERROR(ENOMEM);
            cfg_in = cfg_in_dyn;
        } else
        ...
        if (ctx->nb_outputs > FF_ARRAY_ELEMS(cfg_out_stack)) {
            cfg_out_dyn =
                av_malloc_array(ctx->nb_outputs, sizeof(*cfg_out_dyn));
            if (!cfg_out_dyn) {
                av_freep(&cfg_in_dyn);
                return AVERROR(ENOMEM);
            }
 ...
 */
static void describe_link_formats_cfg(AVFilterLink * link,
                                      AVFilterFormatsConfig * cfg,
                                      AVBPrint * bp)
{
    if (!link || !cfg)
    ...
    if (cfg->formats && cfg->formats->nb_formats > 0) {
        ...
        for (unsigned i = 0; i < cfg->formats->nb_formats; i++) {
            ...
            if (link->type == AVMEDIA_TYPE_VIDEO) {
                av_bprintf(bp, "%s",
                           av_get_pix_fmt_name(cfg->formats->formats[i]));
            } else if (link->type == AVMEDIA_TYPE_AUDIO) {
                av_bprintf(bp, "%s",
                           av_get_sample_fmt_name(cfg->formats->
                                                  formats[i]));
            } else {
    ...
    if (link->type == AVMEDIA_TYPE_AUDIO) {
        ...
        if (cfg->channel_layouts
            && cfg->channel_layouts->nb_channel_layouts > 0) {
            av_bprintf(bp, "[");
            for (int i = 0; i < cfg->channel_layouts->nb_channel_layouts;
                 i++) {
                if (i > 0)
                    av_bprintf(bp, ", ");
                char buf[256];
                av_channel_layout_describe(&cfg->channel_layouts->
                                           channel_layouts[i], buf,
                                           sizeof(buf));
                av_bprintf(bp, "%s", buf);
            }
            av_bprintf(bp, "]");
        } else if (cfg->channel_layouts
                   && cfg->channel_layouts->all_layouts) {
            av_bprintf(bp, "all");
        } else {
            av_bprintf(bp, "none");
        }
        ...
        if (cfg->color_spaces && cfg->color_spaces->nb_formats > 0) {
            ...
            for (unsigned i = 0; i < cfg->color_spaces->nb_formats; i++) {
                if (i > 0)
                    av_bprintf(bp, ", ");
                av_bprintf(bp, "%s",
                           av_color_space_name(cfg->color_spaces->
                                               formats[i]));
            }
            av_bprintf(bp, "]");
        } else {
        ...
        if (cfg->color_ranges && cfg->color_ranges->nb_formats > 0) {
            ...
            for (unsigned i = 0; i < cfg->color_ranges->nb_formats; i++) {
                if (i > 0)
                    av_bprintf(bp, ", ");
                av_bprintf(bp, "%s",
                           av_color_range_name(cfg->color_ranges->
                                               formats[i]));
            }
            av_bprintf(bp, "]");
        } else {
        ...
        for (j = 0; j < filter->nb_inputs; j++) {
            ...
            if (convert_needed) {
                ...
                /* couldn't merge format lists. auto-insert conversion filter */
                {
                    ...
                    av_bprint_init(&src_formats, 0,
                                   AV_BPRINT_SIZE_AUTOMATIC);
                    av_bprint_init(&dst_formats, 0,
                                   AV_BPRINT_SIZE_AUTOMATIC);

                    describe_link_formats_cfg(link, &link->outcfg,
                                              &src_formats);
                    describe_link_formats_cfg(link, &link->incfg,
                                              &dst_formats);
                    ...
                    av_log(log_ctx, AV_LOG_INFO,
                           ...
                           neg->conversion_filter, link->src->name,
                           link->dst->name,
                ...
                if (!
                    (filter =
                     avfilter_get_by_name(neg->conversion_filter))) {
                    av_log(log_ctx, AV_LOG_ERROR,
                ...
                opts =
                    FF_FIELD_AT(char *, neg->conversion_opts_offset,
                                *graph);
                ret =
                ...
                if (ret < 0)
                    return ret;
                if ((ret =
                     avfilter_insert_filter(link, convert, 0, 0)) < 0)
                    return ret;
                ...
                if (outlink->type == AVMEDIA_TYPE_VIDEO) {
                    ...
                    av_assert0(inlink->incfg.channel_layouts->refcount >
                               0);
                    av_assert0(inlink->outcfg.channel_layouts->refcount >
                               0);
                    av_assert0(outlink->incfg.channel_layouts->refcount >
                               0);
                    av_assert0(outlink->outcfg.channel_layouts->refcount >
                               0);
                }
#define MERGE(merger, link)                                                  \
    ((merger)->merge(FF_FIELD_AT(void *, (merger)->offset, (link)->incfg),   \
                ...
                for (neg_step = 0; neg_step < neg->nb_mergers; neg_step++) {
                    const AVFilterFormatsMerger *m =
                        &neg->mergers[neg_step];
                    if ((ret = MERGE(m, inlink)) <= 0
                            ...
                            return ret;
                        {
                            ...
                            av_bprint_init(&inlink_formats, 0,
                                           AV_BPRINT_SIZE_AUTOMATIC);
                            av_bprint_init(&outlink_formats, 0,
                                           AV_BPRINT_SIZE_AUTOMATIC);

                            describe_link_formats_cfg(inlink,
                                                      &inlink->outcfg,
                                                      &inlink_formats);
                            describe_link_formats_cfg(outlink,
                                                      &outlink->incfg,
                                                      &outlink_formats);
                            ...
                            av_log(log_ctx, AV_LOG_ERROR,
                                   ...
                                   link->src->name, link->dst->name,
                                   link->src->name,
                                   inlink_formats.str ? inlink_formats.
                                   str : "unknown", link->dst->name,
                                   outlink_formats.str ? outlink_formats.
                                   str : "unknown");
    ...
    av_log(graph, AV_LOG_DEBUG, "query_formats: "
           ...
           count_queried, count_merged, count_already_merged,
           count_delayed);
    if (count_delayed) {
        AVBPrint bp;
                         ...
                         enum AVSampleFormat src_fmt)
{
    ...
    if (av_get_bytes_per_sample(dst_fmt) <
...
static int pick_format(AVFilterLink * link, AVFilterLink * ref)
{
    ...
    if (link->type == AVMEDIA_TYPE_VIDEO) {
        if (ref && ref->type == AVMEDIA_TYPE_VIDEO) {
            ...
            for (i = 0; i < link->incfg.formats->nb_formats; i++) {
                ...
                best =
                    av_find_best_pix_fmt_of_2(best, p, ref->format,
                                              has_alpha, NULL);
            }
            av_log(link->src, AV_LOG_DEBUG,
    ...
    if (link->type == AVMEDIA_TYPE_VIDEO) {
        ...
        ret =
            av_channel_layout_copy(&link->ch_layout,
                                   &link->incfg.channel_layouts->
                                   channel_layouts[0]);
        if (ret < 0)
            return ret;
    }
...
static int reduce_formats_on_filter(AVFilterContext * filter)
{
    ...
    for (i = 0; i < filter->nb_inputs; i++) {
        ...
        for (j = 0; j < filter->nb_outputs; j++) {
            ...
            if (inlink->type != outlink->type
                || fmts->nb_channel_layouts == 1)
                continue;

            if (fmts->all_layouts && (KNOWN(fmt) || fmts->all_counts)) {
                /* Turn the infinite list into a singleton */
                fmts->all_layouts = fmts->all_counts = 0;
                ret =
                    ff_add_channel_layout(&outlink->incfg.channel_layouts,
                                          fmt);
                if (ret < 0)
                    return ret;
                ret = 1;
            ...
            for (k = 0;
                 k < outlink->incfg.channel_layouts->nb_channel_layouts;
                 k++) {
                if (!av_channel_layout_compare
                    (&fmts->channel_layouts[k], fmt)) {
                    ret =
                        av_channel_layout_copy(&fmts->channel_layouts[0],
                                               fmt);
                    if (ret < 0)
                        return ret;
                    fmts->nb_channel_layouts = 1;
...
static void swap_samplerates_on_filter(AVFilterContext * filter)
{
    ...
    for (i = 0; i < filter->nb_outputs; i++) {
        ...
        for (j = 0; j < outlink->incfg.samplerates->nb_formats; j++) {
            int diff =
                abs(sample_rate - outlink->incfg.samplerates->formats[j]);
...
static void swap_channel_layouts_on_filter(AVFilterContext * filter)
{
    ...
    for (i = 0; i < filter->nb_outputs; i++) {
        ...
        for (j = 0; j < outlink->incfg.channel_layouts->nb_channel_layouts;
             j++) {
            ...
            av_channel_layout_copy(&in_chlayout,
                                   &link->outcfg.channel_layouts->
                                   channel_layouts[0]);
            av_channel_layout_copy(&out_chlayout,
            ...
            if (av_channel_layout_index_from_channel
                (&in_chlayout, AV_CHAN_LOW_FREQUENCY) >= 0
            ...
            matched_channels =
                av_popcount64(in_chlayout.u.mask & out_chlayout.u.mask);
            extra_channels =
                av_popcount64(out_chlayout.u.mask & (~in_chlayout.u.mask));
        ...
        FFSWAP(AVChannelLayout,
               outlink->incfg.channel_layouts->channel_layouts[0],
...
static int pick_formats(AVFilterGraph * graph)
{
    ...
    do {
        ...
        for (i = 0; i < graph->nb_filters; i++) {
            ...
            if (filter->nb_inputs) {
                for (j = 0; j < filter->nb_inputs; j++) {
                    if (filter->inputs[j]->incfg.formats
                        && filter->inputs[j]->incfg.formats->nb_formats ==
                        1) {
                        if ((ret =
                             pick_format(filter->inputs[j], NULL)) < 0)
                            return ret;
                        change = 1;
                    }
                }
            ...
            if (filter->nb_outputs) {
                for (j = 0; j < filter->nb_outputs; j++) {
                    if (filter->outputs[j]->incfg.formats
                        && filter->outputs[j]->incfg.formats->nb_formats ==
                        1) {
                        if ((ret =
                             pick_format(filter->outputs[j], NULL)) < 0)
                            return ret;
                        change = 1;
                    }
                }
                                ...
                                char *res, int res_len, int flags)
{
    ...
    if ((flags & AVFILTER_CMD_FLAG_ONE)
        && !(flags & AVFILTER_CMD_FLAG_FAST)) {
        r = avfilter_graph_send_command(graph, target, cmd, arg, res,
                                        res_len,
                                        flags | AVFILTER_CMD_FLAG_FAST);
        if (r != AVERROR(ENOSYS))
            return r;
    ...
    for (i = 0; i < graph->nb_filters; i++) {
        ...
        if (!strcmp(target, "all")
            ...
            r = avfilter_process_command(filter, cmd, arg, res, res_len,
                                         flags);
            if (r != AVERROR(ENOSYS)) {
                                 ...
                                 int flags, double ts)
{
    ...
    for (i = 0; i < graph->nb_filters; i++) {
        ...
        if (filter
            && (!strcmp(target, "all") || !strcmp(target, filter->name)
                || !strcmp(target, filter->filter->name))) {
...
int avfilter_graph_request_oldest(AVFilterGraph * graph)
{
    ...
    while (graphi->sink_links_count) {
        ...
        av_log(oldest->dst, AV_LOG_DEBUG, "EOF on sink link %s:%s.\n",
               oldest->dst->name, oldest->dstpad->name);
